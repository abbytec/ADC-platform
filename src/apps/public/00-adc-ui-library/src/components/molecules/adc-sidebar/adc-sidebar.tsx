...
export class AdcSidebar {
	...
	render() {
		...
		return (
			<aside
				class={`z-50 fixed px-2 pt-5 left-0 top-auto h-full bg-background text-primary transition-all duration-300 shadow-[0_5px_20px_rgba(0,0,0,0.15)] overflow-hidden ${sidebarClass}`}
				role="complementary"
			>
				{(this.title || this.subtitle) && (
					<div
						class={`flex flex-col justify-center items-center px-3 mb-4 transition-opacity duration-300 ${this.collapsed ? "opacity-0 pointer-events-none lg:opacity-100 lg:pointer-events-auto" : "opacity-100"}`}
					>

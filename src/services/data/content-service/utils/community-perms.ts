import { P, hasPermissionString } from "@common/types/Permissions.ts";
...
export function canPublish(user: UserLike | null | undefined): boolean {
	...
	return hasPermissionString(user.permissions, P.COMMUNITY.PUBLISH_STATUS.WRITE);
}
...
export function canModerateSocial(user: UserLike | null | undefined): boolean {
	if (!user?.permissions) return false;
	return hasPermissionString(user.permissions, P.COMMUNITY.SOCIAL.DELETE);
}

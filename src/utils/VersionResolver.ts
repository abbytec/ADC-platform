...
export class VersionResolver {
  ...
  static async resolveModuleVersion(
    ...
    try {
      ...
      let compatible = candidates.filter(c =>
        this.satisfiesRange(c.version, versionRange)
      );

      if (compatible.length === 0) {
        Logger.warn(
          `[VersionResolver] No hay versión compatible de ${moduleName} para el rango ${versionRange}`
        );
        return null;
      }
